...
<%
	...
	if (searchKey != null)			//-- 검색 기능을 통해 이 페이지가 요청되었을 경우
	{
		...
		// (A 페이지에서 B페이지로 값을 넘김 그러면 A 페이지 인코딩이 있고 B 페이지 인코딩이 있을텐데 body 안에서 넘겨지니까
		//  get 방식은 url 쪽으로 넘겨서 키=값&키=값 이렇게 넘김 별도의 인코딩 방식이 적용된다 
		//  그래서 원래 페이지 인코딩으로 지정해야 하는데 지원하지 않는 인코딩 방식이라면 예외가 발생하는 구조로
		//  짜여있다. 그래서 별도로 디코딩에 대한 처리 해야한다.)
      ...
      <div id="leftHeader">
		...
		<form action="" name="searchForm" method="post">
		   	<select name="searchKey" class="selectFiled">
				...
				else									// 수신한 searchKey가 subject이거나... 없으면...
				{
				%>
					<option value="subject">제목</option>
					<option value="name">작성자</option>
